...
class SaleOrderCreateWizard(models.Model):
    ...
    def default_get(self, fields):
        ...
        coa_id = self.env['account.account'].search(
            [('name', '=', 'Petty Cash'), ('company_ids', 'in', self.env.company.id)])
        for i in pump_entry_line:
            line = (0, 0, {
                ...
                'payment_mode': i.payment_mode.id,
                'record_type': 'advance',
                'payment_create': True,
                'default': True,
            })
    ...
    def action_create_return_order(self):
        ...
        stock_return_picking = return_picking_obj.create({
            'picking_id': self.picking_id.id,
        })

        for move in stock_return_picking.product_return_moves:
            if self.product_id.product_variant_id == move.product_id:
                move.quantity = self.test_sale_value
            else:
                move.quantity = 0.00
        ...
        if return_pick.state not in ['done', 'cancel']:
    ...
    def _create_credit_payment(self):
        for cp in self.credit_payment_ids:
            vals = {
                ...
                'memo': 'Credit Sale Payment (' + cp.customer_id.name + ')',
    ...
    def action_create_sale_order(self):
        if self.balance_price == 0.00:
            ...
            self.balance_payment_entry(partner)
            for invoice in sale.invoice_ids:
                ...
                if advance_pay:
                    for aa in advance_pay:
                        if aa.state == 'paid' and aa.amount != 0.00:
    ...
    def balance_payment_entry(self, partner):
        for i in self:
            total_expense = sum(item.amount for item in i.expense_ids)
            vals = {
                'payment_type': 'inbound',
                'amount': total_expense,
                'journal_id': self.env.ref('account.1_bank').id,
                'memo': 'Balance Advance',
                'entry_type': 'advance',
                'partner_id': partner.id,
                'pay_ref': self.id,
                'date': self.date.date(),
    ...
    def create(self, values):
        values[0]['name'] = self.sudo().env['ir.sequence'].next_by_code('sale.order.wizard') or 'New'
...
class SaleOrderCreateWizardLine(models.Model):
    ...
    oil_id = fields.Many2one('product.product', string='Product',
