...
class PetrolStation(models.Model):
    ...
    product_id = fields.Many2one('product.template', string='Fuel ',
                                 domain=[('sale_ok', '=', True), ('type', '=', 'consu'),('is_storable', '=', True)])
    ...
    def fetch_data(self):
        ...
        petty_cash = self.env.user.has_group('petrol_station_dashboard.group_petty_cash')
        payment_mode = self.env['account.journal'].search([('type', 'in', ['cash', 'bank'])])

        support_person = self.env.user.has_group('petrol_station_dashboard.group_supportive_person')
        for machine in machines:
            if machine.old_reading_update_time:
                ott = machine.old_reading_update_time + timedelta(hours=5, minutes=30)
            if machine.employee_id.contract_id.petty_cash_received:
                petty_cash_status = "true"
            else:
                petty_cash_status = "false"
            data.append({
                'petty_cash_received': petty_cash_status,
            ...
            val['payment_mode'] = [{'id': journal.id, 'name': journal.name} for journal in payment_mode]
    ...
    def create_journal_entry(self, id, petty_cash_amt, employee):
        line_vals = []
        coa_id = self.env['account.account'].search([('code', '=', '101100'), ('name', '=', 'Petty Cash')])
...
class PetrolStationLine(models.Model):
    ...
    def create_record(self, dict):
        ...
        if dict:
            ...
            self.env['petrol.station.pump'].search([('id', '=', dict['id'])])
